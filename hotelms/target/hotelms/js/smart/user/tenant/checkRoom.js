...
var requestRoomPic = function(room_id){
    ...
    $.ajax({
        url:'/SMART/userRoom/getRoomPic.action',
        data:'id=' + room_id,
        success:function(data){
            ...
            for(var i=0; i<data.length; i++){
                ...
                $(".big_pic ul").append("<li><img src= '"+window.document.location.origin +"/SMART" + data[i].path +"' /></li>");
                $(".num ul").append("<li class='linormal' onclick='changeMinPic(this);'><img src= '"+window.document.location.origin +"/SMART" + data[i].path +"' /></li>");
            }
...
var getRoomIDByNumber = function(roomNumber){
    ...
    $.ajax({
        url:"/SMART/userRoom/getRoomIDByNumber.action",
