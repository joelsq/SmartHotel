...
var searchExpense = function () {
    ...
    $.ajax({
        type:'post',
        url:'/SMART/userRoom/getAllStatistics.action',
...
var exportFunction = function () {
    ...
    $.ajax({
        type:'post',
        url:'/SMART/userRoom/exportExpenseToModel.action',
        contentType:'application/json',
        dataType:'json',
        ...
        success:function(data){
            if(data == 1){
                window.location.href = "http://" + window.location.host + '/SMART/download.action?fp=excel/Consumption.xlsx';
            }else{
