...
import team.smart.service.ProblemService;
import team.smart.service.ReminderService;
...
public class ProblemController {
	...
	public @ResponseBody Map<String, Object> searchProblemsByPage(HttpSession session,
	...
	public @ResponseBody Integer addProblemByOutsider(HttpSession session, @RequestBody Map<String, Object> params)
	...
	public @ResponseBody Integer addProblem(HttpSession session, @RequestBody Map<String, Object> params)
			throws ParseException {
		User curUser = (User) session.getAttribute("curUser");
		// if ((curUser.getAUTH() & (0x01 << Config.getAuths().get("rwRate"))) == 0) {
		// return 0;
	...
	public @ResponseBody Integer dealProblem(HttpSession session, @RequestBody Map<String, Object> params)
			throws ParseException {
		session.getAttribute("curUser");
	...
	public String uploadProblemPicture(@RequestParam("file") MultipartFile[] file, Integer problemId,
				...
				if (!mul.isEmpty()) {
					Streams.copy(mul.getInputStream(),
							new FileOutputStream(request.getSession().getServletContext().getRealPath("")
									+ "/resources/" + foldersName + "/" + problemId
									+ mul.getOriginalFilename().substring(mul.getOriginalFilename().lastIndexOf("."))),
							true);
				}
	...
	public String uploadProblemPictureByDeal(@RequestParam("file") MultipartFile[] file, Integer problemId,
			HttpServletRequest request) {
		// System.out.println(request.getSession().getServletContext().getRealPath(""));
		System.out.println("problemId：" + problemId);
	...
	public Integer addProblemReply(HttpSession session, @RequestBody String data) {
		...
		try {
			...
			dataJson.getString("thisMonthDay");
			dataJson.getString("alreadyPay");
